...
import React, { type FC, useMemo, useContext, useRef } from "react";
import {
...
export const ChunkMesh: FC<{
  chunk: IcoChunk;
  tilePositionAttribute: BufferAttribute;
}> = ({ chunk, tilePositionAttribute }) => {
  ...
  const indices = useMemo(() => {
    const result = new Uint32Array(chunk.triangles.length * 3);
    let i = 0;
  ...
  //  const tilePositionAttributeRef = useRef(tilePositionAttribute);
...
/*const {
  ...
  for (const tri of chunk.triangles) {
    ...
    lngLatArr.push(
      new Vector2(wrapRightToLeft(tri.a.lngLat.x, doesWrap), tri.a.lngLat.y),
      new Vector2(wrapRightToLeft(tri.b.lngLat.x, doesWrap), tri.b.lngLat.y),
      new Vector2(wrapRightToLeft(tri.c.lngLat.x, doesWrap), tri.c.lngLat.y),
    );
  ...
  return {
    indices: new Uint32Array(
      chunk.triangles.flatMap(({ a, b, c }) => [a.index, b.index, c.index]),
    ),
    positions,
    lngLats,
    colors,
